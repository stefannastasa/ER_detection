...
from utils.annotations import corner_annotator

# define input directory and filename
input_folder = arrays_folder
...
for case in cases:
    ...
    if case in point_cases.keys():
        ...
        # find clips
        frame_folder = os.path.join(input_folder, case)
        ...
        while len(frames) != 0:
            ...
            array = np.load(os.path.join(frame_folder, frames[idx])).astype(np.float32)
            ...
            if points != None:
                ...
                point_clips[frame_name] = points
                prev_annotation = points
                frames.remove(frames[idx])
            else:
                idx += 1
