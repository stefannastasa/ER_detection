...
class CornerAnnotationEventTracker:
    ...
    def keypress(self, event: matplotlib.backend_bases.KeyEvent) -> None:
        ...
        if self.polar:
            ...
            # decrease the image padding by one step
                ...
                if self.pad != 0:
                    self.pad -= self.padding_step
                    self.points = [(point[0] - self.padding_step, point[1] - self.padding_step) for point in
                                   self.points]
                    for point in self.point_objs:
                        point.remove()
                    for line in self.line_objs:
    ...
    def change_ax(self) -> None:
        ...
        """
        self.padded_image_array = np.pad(self.image_array, [(0, 0), (self.pad, self.pad), (self.pad, self.pad)],
                                         mode='constant')
        self.image.remove()
        self.image = self.ax.imshow(self.padded_image_array[self.idx, :, :], vmin=self.vmin, vmax=self.vmax,
                                    cmap=self.cmap)
    ...
    def update(self) -> None:
        ...
        while len(self.points) // 2 > len(self.line_objs):
            ...
            l = self.ax.plot([self.points[i][0], self.points[i + 1][0]], [self.points[i][1], self.points[i + 1][1]],
                             color='red', lw=0.75)
...
class ScaleAnnotationEventTracker:
    ...
    def update(self) -> None:
        ...
        if len(self.points) == 2 and self.line_obj == []:
            self.line_obj.append(
                self.ax.plot([point[0] for point in self.points], [point[1] for point in self.points], color='red')[0])
